...
const renderMarkdownContent = (content) => {
  ...
  // 情况 A：如果已经成功转换成了新版 Strapi Blocks (JSON 数组)
    ...
    content.forEach(block => {
      ...
      // 2. 处理无序列表 (List)
        ...
        block.children?.forEach(item => {
          ...
          item.children?.forEach(child => {
            // 如果声明了 bold: true，则包裹 strong 标签实现局部加粗
            if (child.bold) {
              htmlResult += `<strong>${child.text}</strong>`
            } else {
              htmlResult += child.text
            }
          })
          htmlResult += '</li>'
        })
      ...
      // 3. 处理普通段落 (Paragraph)
        ...
        block.children?.forEach(child => {
          if (child.bold) {
            paraHtml += `<strong>${child.text}</strong>`
          } else {
            paraHtml += child.text
          }
        })
