...
// 2. 网络请求：使用动态箭头函数，并关闭 initialCache 缓存，防止 Vercel 锁死空数据
...
// 提取纯文本（专门用于 SEO 标签，不含任何 HTML 标签符号）
...
// 【核心优化】：利用计算属性，专门将 Markdown 纯文本实时翻译编译为网页真正的 HTML 标签
const renderedDescriptionHtml = computed(() => {
  ...
  // 用 snarkdown 库把后台带 ### 和 ** 的字符串变成真正网页认识的 <h3> <ul> <strong> 标签
...
/* ==========================================================================
   🎨 Markdown 样式自定义微调（确保前端生成的列表点、加粗和标题有明显的层次和间距）
   ========================================================================== */
