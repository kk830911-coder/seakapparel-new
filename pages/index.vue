...
// 新增：获取纯净无参数图片地址，剥离所有query，交给NuxtImage全局配置处理格式
const getCleanImageUrl = (item) => {
  ...
  // Cloudinary地址剥离全部查询参数，只保留基础路径
  return url.split('?')[0]
}

// 生成300px缩略图srcset，仅保留裁切压缩，移除f=avif/fl_any_format
const getThumb300 = (url) => {
  if (!url || !url.includes('res.cloudinary.com')) return url
  const cleanUrl = url.split('?')[0]
  return `${cleanUrl}?w=300&h=300&c_fill&q=75&f_auto`
}

// 生成600px商品主图，仅裁切+画质，不强制avif，由nuxt.config全局format控制
const getOptimizedUrl = (url) => {
  const fallbackImg = 'https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?w=600&h=600&c_fill&q=75&f_auto'
  if (!url) return fallbackImg

  if (url.includes('res.cloudinary.com')) {
    const cleanUrl = url.split('?')[0]
    return `${cleanUrl}?w=600&h=600&c_fill&q=75&f_auto`
  }
  return url
}
  ...
  >
    ...
    <section class="max-w-[1400px] mx-auto px-[5px] md:px-4 pb-16">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        ...
        <div class="relative overflow-hidden rounded-lg">
          <NuxtImg
            ...
            loading="lazy"
          />
          <div class="absolute bottom-0 left-0 right-0 py-2 text-center text-white font-medium tracking-wider z-10" style="background-color:rgba(157, 37, 218, 0.85)">Design & Sample Making</div>
        </div>
        <div class="relative overflow-hidden rounded-lg">
          <NuxtImg
            src="/gongchang_03.jpg"
            format="avif"
            fallbackFormat="webp"
            alt="Ironing Area"
            class="w-full h-64 object-cover"
            loading="lazy"
          />
          <div class="absolute bottom-0 left-0 right-0 py-2 text-center text-white font-medium tracking-wider z-10" style="background-color:rgba(157, 37, 218, 0.85)">Ironing Zone</div>
        </div>
        <div class="relative overflow-hidden rounded-lg">
          <NuxtImg
            src="/gongchang_04.jpg"
            format="avif"
            fallbackFormat="webp"
            alt="Cutting Area"
            class="w-full h-64 object-cover"
            loading="lazy"
          />
          <div class="absolute bottom-0 left-0 right-0 py-2 text-center text-white font-medium tracking-wider z-10" style="background-color:rgba(157, 37, 218, 0.85)">Cutting Workshop</div>
        </div>
        <div class="relative overflow-hidden rounded-lg">
          <NuxtImg
            src="/gongchang_05.jpg"
            format="avif"
            fallbackFormat="webp"
            alt="Sewing Workshop"
            class="w-full h-64 object-cover"
            loading="lazy"
          />
          <div class="absolute bottom-0 left-0 right-0 py-2 text-center text-white font-medium tracking-wider z-10" style="background-color:rgba(157, 37, 218, 0.85)">Sewing Workshop</div>
        </div>
        <div class="relative overflow-hidden rounded-lg">
          <NuxtImg
            src="/gongchang_06.jpg"
            format="avif"
            fallbackFormat="webp"
            alt="Packaging Area"
            class="w-full h-64 object-cover"
            loading="lazy"
          />
          <div class="absolute bottom-0 left-0 right-0 py-2 text-center text-white font-medium tracking-wider z-10" style="background-color:rgba(157, 37, 218, 0.85)">Final Packaging</div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="relative overflow-hidden rounded-lg">
          <NuxtImg
            src="/gongchang_08.jpg"
            format="avif"
            fallbackFormat="webp"
            alt="Warehouse Corner"
            class="w-full h-64 object-cover"
            loading="lazy"
          />
          <div class="absolute bottom-0 left-0 right-0 py-2 text-center text-white font-medium tracking-wider z-10" style="background-color:rgba(157, 37, 218, 0.85)">Warehouse Corner</div>
        </div>
        <!-- 中间紫色文字块 -->
    ...
    <section class="max-w-[1400px] mx-auto px-[5px] lg:px-4 pb-16">
      ...
      <div v-else class="grid grid-cols-2 lg:grid-cols-4 gap-[10px] lg:gap-[15px]">
        <div
        ...
        >
          ...
          <!-- 卡片内部上下左右内边距5px -->
          <div class="p-[5px] flex-1 flex flex-col justify-between">
            <NuxtLink
              :to="`/products/${item.slug || item.attributes?.slug}`"
              target="_blank"
              class="block"
    ...
    <!-- 定制流程板块：手机左右5px边距 -->
    <section class="py-16 px-[5px] md:px-4" style="background-color:#9d25da;">
      <div class="max-w-[1400px] mx-auto">
        ...
        <!-- 仅修改此处class：grid-cols-2 手机一行2个卡片，电脑lg:grid-cols-6保持6列不变 -->
        <div class="grid grid-cols-2 lg:grid-cols-6 gap-4">
          ...
          <!-- Step5 Shipment -->
          <div class="bg-white rounded-lg p-6 text-center">
            <svg width="70" height="70" viewBox="0 0 24 24" fill="none" stroke="#9d25da" stroke-width="2.5" class="mx-auto mb-4">
